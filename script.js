...
(function () {
  ...
  const pct = (cur, tgt) => (tgt > 0 ? clamp(Math.round((cur / tgt) * 100), 0, 100) : 0);
  ...
  const AREA_COLOR = {
    health: "var(--green)", career: "var(--purple)", learning: "var(--cyan)",
    spirituality: "var(--amber)", finance: "var(--yellow)", relationships: "var(--pink)",
  };
  const CATEGORIES = ["Health", "Career", "Learning", "Finance", "Spirituality", "Relationships", "Discipline"];
  const GOAL_TYPES = [
  ...
  const DEFAULT_STATE = {
    user: { name: "Arham Tanveer", level: 12, title: "Warrior", xp: 820, xpMax: 1200 },
    settings: { theme: "dark", accent: "aurora", animations: true },
    achievements: [],
    history: {},
    yesterdayScore: 65,
    goals: [
      { id: "g1", emoji: "🥤", title: "Morning Shake",   subtitle: "Drink your protein shake", type: "checkbox", unit: "",    target: 1,    current: 1,    points: 10, category: "Health",       color: "cyan"   },
      { id: "g2", emoji: "💧", title: "Drink Water",      subtitle: "Stay hydrated",           type: "water",    unit: "ml",  target: 3000, current: 2400, points: 10, category: "Health",       color: "blue"   },
      { id: "g3", emoji: "💻", title: "Learn JavaScript", subtitle: "Coding practice",         type: "minutes",  unit: "min", target: 120,  current: 105,  points: 20, category: "Learning",     color: "purple" },
      { id: "g4", emoji: "💼", title: "Apply to Jobs",    subtitle: "Apply to at least 10 jobs", type: "count",  unit: "",    target: 10,   current: 6,    points: 15, category: "Career",       color: "amber"  },
      { id: "g5", emoji: "📖", title: "Study Islamiat",   subtitle: "Study for knowledge",     type: "minutes",  unit: "min", target: 15,   current: 13,   points: 15, category: "Spirituality", color: "green"  },
      { id: "g6", emoji: "🏋️", title: "Workout",          subtitle: "45 minutes workout",      type: "minutes",  unit: "min", target: 45,   current: 31,   points: 30, category: "Health",       color: "pink"   },
    ],
    streaks: [
    ...
    weekly: [
      { day: "Mon", value: 72 }, { day: "Tue", value: 58 }, { day: "Wed", value: 41 },
      { day: "Thu", value: 60 }, { day: "Fri", value: 83 }, { day: "Sat", value: 90 }, { day: "Sun", value: 68 },
    ],
    lifeAreas: [
      { key: "health", label: "Health", value: 75 }, { key: "career", label: "Career", value: 65 },
      { key: "learning", label: "Learning", value: 80 }, { key: "spirituality", label: "Spirituality", value: 70 },
      { key: "finance", label: "Finance", value: 60 }, { key: "relationships", label: "Relationships", value: 50 },
    ],
  ...
  function weekDates(offsetWeeks = 0) {
    ...
    const monday = new Date(now); monday.setDate(now.getDate() - mondayIdx + offsetWeeks * 7);
  ...
  function weeklyData(offsetWeeks = 0) {
    ...
    return weekDates(offsetWeeks).map((d, i) => {
      const key = dateKey(d);
      return { day: WD[i], date: key, value: Store.state.history[key] ?? 0, future: key > today };
    });
  }
  function weekdayAverages() {
    ...
    return WD.map((label, i) => ({ label, avg: cnt[i] ? Math.round(sums[i] / cnt[i]) : 0 }));
  }
  function lastNDays(n) {
    ...
    for (let i = n - 1; i >= 0; i--) {
      const d = new Date(today); d.setDate(today.getDate() - i);
      out.push({ date: dateKey(d), value: Store.state.history[dateKey(d)] ?? 0 });
    }
    return out;
  }
  ...
  const Store = {
    ...
    /* mutations */
    addGoal(goal) { this.state.goals.push({ ...goal, id: "g" + Date.now() }); this.emit(); },
    updateGoal(id, patch) {
      const g = this.state.goals.find(x => x.id === id);
      if (g) Object.assign(g, patch);
      this.emit();
    },
    removeGoal(id) { this.state.goals = this.state.goals.filter(x => x.id !== id); this.emit(); },
    toggleGoal(id) {
      const g = this.state.goals.find(x => x.id === id);
      if (!g) return;
    ...
    setTheme(theme) { this.state.settings.theme = theme; this.emit(); },
    setAccent(accent) { this.state.settings.accent = accent; this.emit(); },
    setAnimations(on) { this.state.settings.animations = on; this.emit(); },
    setUserName(name) { this.state.user.name = name; this.emit(); },
    /* persistence */
    save() {
      this.state.history[dateKey()] = todayScore();          // record today's score for charts/heatmap
      try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ v: 1, state: this.state })); } catch (e) { /* storage full / disabled */ }
    },
    load() {
      try {
      ...
      } catch (e) { /* corrupt data — keep defaults */ }
    },
    replaceState(obj) { this.state = deepMerge(structuredClone(DEFAULT_STATE), obj); this.emit(); },
    reset() {
  ...
  function todayScore() {
    ...
    const earned = goals.reduce((s, g) => s + (g.points || 0) * (pct(g.current, g.target) / 100), 0);
    return Math.round((earned / totalPoints) * 100);
  }

  /* ---------- Render: sidebar user + quote ---------- */
  function renderUser(s) {
    $(".greeting__title").innerHTML = `Good ${greetingWord()}, ${escapeHtml(s.user.name)} <span aria-hidden="true">👋</span>`;
    $(".avatar__initials").textContent = initials(s.user.name);
    $(".level-card__level").textContent = "Level " + s.user.level;
    $(".level-card__title").textContent = s.user.title;
    $(".level-card__xp-value").textContent = `${s.user.xp} / ${s.user.xpMax} XP`;
    const xpFill = $(".progress--xp .progress__fill");
    xpFill.style.setProperty("--_p", pct(s.user.xp, s.user.xpMax));
    $(".quote-card__text").innerHTML = `"${escapeHtml(s.quote)}" <span aria-hidden="true">⚡</span>`;
  ...
  function renderRing(score, { animate = false } = {}) {
    ...
    ringVal.parentElement.setAttribute("aria-label", `Today's completion ${score} percent`);
  ...
  function renderCompare(score, s) {
    ...
    values[0].textContent = s.yesterdayScore + "%";
    values[1].textContent = score + "%";
    const delta = score - s.yesterdayScore;
    const up = delta >= 0;
    ...
    deltaEl.innerHTML = `<span aria-hidden="true">${up ? "↑" : "↓"}</span> ${Math.abs(delta)}% ${up ? "Improvement" : "Dip"}`;
    $(".compare__message", card).textContent = up
      ? "You're doing better than yesterday!"
      : "Every day is a fresh chance — keep going.";
  }

  /* ---------- Render: streaks ---------- */
  function renderStreaks(s) {
    const list = $(".streak-list");
    list.innerHTML = s.streaks.map(st => `
      <li class="streak">
        <span class="streak__icon" aria-hidden="true">${st.emoji}</span>
        <span class="streak__name">${escapeHtml(st.name)}</span>
        <span class="streak__count">${st.count} days <span aria-hidden="true">🔥</span></span>
      </li>`).join("");
  }
  ...
  function goalMarkup(g) {
    ...
    const percentCell = isCheckbox ? `<span class="goal__percent"></span>` : `<span class="goal__percent">${p}%</span>`;
    return `
  ...
  function renderWeekly() {
    ...
    bars.innerHTML = data.map(d => `
      <li class="bar ${d.future ? "is-future" : ""}"><span class="bar__fill"></span><span class="bar__label">${d.day}</span></li>`).join("");
    data.forEach((d, i) => {
  ...
  function renderLife(s) {
    ...
    grid.innerHTML = s.lifeAreas.map(a => `
      <li class="life">
  ...
  function renderInsight(score, s) {
    ...
    const bullets = [];
    const topStreak = [...s.streaks].sort((a, b) => b.count - a.count)[0];
    if (topStreak) bullets.push(`${escapeHtml(topStreak.name)} streak is on fire — ${topStreak.count} days <span aria-hidden="true">🔥</span>`);
    const scored = s.goals.map(g => ({ g, p: pct(g.current, g.target) }));
    const lowest = scored.sort((a, b) => a.p - b.p)[0];
    if (lowest && lowest.p < 100) bullets.push(`${escapeHtml(lowest.g.title)} needs attention — ${lowest.p}% done`);
    const doneCount = scored.filter(x => x.p >= 100).length;
    bullets.push(`${doneCount} of ${s.goals.length} goals fully complete today`);

    $(".insight__list").innerHTML = bullets.map(b => `<li>${b}</li>`).join("");
  }
  ...
  function computeAnalytics() {
    ...
    const avg30 = tracked30.length ? Math.round(tracked30.reduce((a, b) => a + b, 0) / tracked30.length) : 0;
    const entries = Object.entries(Store.state.history);
    let best = { date: "—", value: 0 };
    entries.forEach(([k, v]) => { if (v > best.value) best = { date: k, value: v }; });
    const thisW = weeklyData(0).filter(d => !d.future).map(d => d.value);
    const lastW = weeklyData(-1).map(d => d.value);
    const wa = arr => arr.length ? Math.round(arr.reduce((a, b) => a + b, 0) / arr.length) : 0;
    return {
      avg30, best,
      daysTracked: entries.length,
      longestStreak: Math.max(0, ...Store.state.streaks.map(s => s.count)),
      thisWeekAvg: wa(thisW), lastWeekAvg: wa(lastW),
    };
  }
  ...
  function buildAreaChart(values, { w = 720, h = 240, pad = 26 } = {}) {
    ...
    const grid = [0, 25, 50, 75, 100].map(g =>
      `<line x1="${pad}" y1="${Y(g).toFixed(1)}" x2="${w - pad}" y2="${Y(g).toFixed(1)}" class="linechart__grid"/>`).join("");
    return `
  ...
  function renderAnalytics() {
    ...
    const weakest = [...wds].filter(d => d.avg > 0).sort((x, y) => x.avg - y.avg)[0];
    ...
    view.querySelectorAll(".js-count").forEach(el => animateCount(el, +el.dataset.to, { suffix: el.dataset.suffix || "" }));
  ...
  const LEVEL_COLOR = { 0: "var(--txt-faint)", 1: "var(--red)", 2: "var(--amber)", 3: "var(--green)", 4: "var(--green)" };
  ...
  function renderHeatmap() {
    ...
    const start = new Date(today); start.setDate(today.getDate() - mondayIdx - 52 * 7);
    const weeks = 53;

    let cells = "", months = "", lastMonth = -1;
    for (let w = 0; w < weeks; w++) {
      const wkDate = new Date(start); wkDate.setDate(start.getDate() + w * 7);
      if (wkDate.getMonth() !== lastMonth) { months += `<span style="grid-column:${w + 1}">${MONTHS[wkDate.getMonth()]}</span>`; lastMonth = wkDate.getMonth(); }
      for (let d = 0; d < 7; d++) {
        const date = new Date(start); date.setDate(start.getDate() + w * 7 + d);
        if (date > today) { cells += `<i class="heatcell is-empty" aria-hidden="true"></i>`; continue; }
  ...
  function updateHeatToday() {
    ...
    if (cell) { const sc = todayScore(); cell.dataset.score = sc; cell.dataset.level = scoreLevel(sc); }
  }
  ...
  function buildDayModal() {
    ...
    dayOverlay.addEventListener("click", (e) => {
      if (e.target === dayOverlay || e.target.closest("[data-close]")) dayOverlay.classList.remove("is-open");
    });
  }
  function openDay(dateStr, score) {
    const d = new Date(dateStr + "T00:00:00");
    const lvl = scoreLevel(score);
    $("#day-title").textContent = d.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" });
    $("#day-score").textContent = score + "%";
    $("#day-band").textContent = bandLabel(score);
    $("#day-band").style.color = LEVEL_COLOR[lvl];
    $("#day-ring").style.background = `conic-gradient(from -90deg, ${LEVEL_COLOR[lvl]} ${score}%, var(--track) 0)`;
  ...
  const A = {
    ...
    streak: (name) => (Store.state.streaks.find(s => s.name.toLowerCase().includes(name)) || { count: 0 }).count,
    ...
    bestRun: (thresh) => {
      ...
      for (const k of keys) {
        ...
        if (Store.state.history[k] >= thresh) { cur = consec ? cur + 1 : 1; best = Math.max(best, cur); } else { cur = 0; }
        prev = d;
      }
      return best;
    },
  ...
  const ACHIEVEMENTS = [
    { id: "first-goal",     icon: "🎯", title: "First Goal",      desc: "Create your first goal",             check: () => A.goals() >= 1 },
    { id: "perfect-day",    icon: "🌟", title: "Perfect Day",     desc: "Hit 100% in a single day",           check: () => A.bestScore() >= 100 },
    { id: "streak-7",       icon: "🔥", title: "7-Day Streak",    desc: "Keep a streak alive for 7 days",     check: () => A.maxStreak() >= 7,  prog: () => A.maxStreak() / 7 },
    { id: "streak-30",      icon: "🏆", title: "30-Day Streak",   desc: "Reach 30 days on one streak",        check: () => A.maxStreak() >= 30, prog: () => A.maxStreak() / 30 },
    { id: "perfect-week",   icon: "📅", title: "Perfect Week",    desc: "7 days in a row above 80%",          check: () => A.bestRun(80) >= 7,  prog: () => A.bestRun(80) / 7 },
    { id: "perfect-month",  icon: "👑", title: "Perfect Month",   desc: "30 days in a row above 80%",         check: () => A.bestRun(80) >= 30, prog: () => A.bestRun(80) / 30 },
    { id: "shake-master",   icon: "🥤", title: "Shake Master",    desc: "10-day Morning Shake streak",        check: () => A.streak("shake") >= 10, prog: () => A.streak("shake") / 10 },
    { id: "coding-warrior", icon: "💻", title: "Coding Warrior",  desc: "14-day Coding streak",               check: () => A.streak("coding") >= 14, prog: () => A.streak("coding") / 14 },
    { id: "job-hunter",     icon: "💼", title: "Job Hunter",      desc: "5-day Job Applications streak",       check: () => A.streak("job") >= 5, prog: () => A.streak("job") / 5 },
    { id: "consistent",     icon: "📈", title: "Consistent",      desc: "Track 30 different days",             check: () => A.daysTracked() >= 30, prog: () => A.daysTracked() / 30 },
    { id: "centurion",      icon: "💯", title: "Centurion",       desc: "Track 100 different days",           check: () => A.daysTracked() >= 100, prog: () => A.daysTracked() / 100 },
    { id: "goal-machine",   icon: "⚙️", title: "Goal Machine",    desc: "Have 8 or more active goals",        check: () => A.goals() >= 8, prog: () => A.goals() / 8 },
  ];
  ...
  function syncAchievements(silent) {
    ...
    const fresh = [];
    ACHIEVEMENTS.forEach(a => { if (a.check() && !known.has(a.id)) { known.add(a.id); fresh.push(a); } });
    Store.state.achievements = [...known];
    if (!silent && fresh.length) fresh.forEach(a => celebrate(a));
    if ($("#view-achievements").classList.contains("is-active")) renderAchievements();
  }
      ...
      <div class="achievements">
        ...
        <div class="badge-grid">
          ${ACHIEVEMENTS.map(a => {
            ...
            const p = !on && a.prog ? clamp(Math.round(a.prog() * 100), 0, 99) : null;
            return `
  ...
  function render() {
    ...
    if (!firstPaint && score === 100 && lastScore !== 100 && typeof window.burstConfetti === "function") window.burstConfetti({ big: true });
    lastScore = score;
    firstPaint = false;
  }
  ...
  /* ---------- Sidebar (mobile) ---------- */
  const app = $(".app");
  function openSidebar()  { app.classList.add("nav-open"); }
  function closeSidebar() { app.classList.remove("nav-open"); }
  ...
  function applyAccent(accent) { document.documentElement.dataset.accent = accent || "aurora"; }
  function applyAnimations(on) { document.documentElement.dataset.animations = on ? "on" : "off"; }
  function applySettings(s) { applyTheme(s.settings.theme); applyAccent(s.settings.accent); applyAnimations(s.settings.animations); }
  function initTheme() {
  ...
  function buildModal() {
    ...
    modalRoot.addEventListener("click", (e) => {
      ...
      if (e.target.closest("[data-delete]")) { Store.removeGoal(editingId); toast("Goal deleted"); closeModal(); }
    });
  ...
  function syncTypeFields() {
    ...
    if (type === "water" && !$("#f-unit", modalRoot).value) $("#f-unit", modalRoot).value = "ml";
    if (type === "minutes" && !$("#f-unit", modalRoot).value) $("#f-unit", modalRoot).value = "min";
  }
  ...
  function closeModal() { modalRoot.classList.remove("is-open"); editingId = null; }
  function saveModal() {
    const title = $("#f-title", modalRoot).value.trim();
    if (!title) { $("#f-title", modalRoot).focus(); return toast("Give your goal a title"); }
    const type = $("#f-type", modalRoot).value;
    const data = {
      title,
      ...
      target: type === "checkbox" ? 1 : Math.max(1, +$("#f-target", modalRoot).value || 1),
    ...
    if (editingId) {
      data.current = clamp(+$("#f-current", modalRoot).value || 0, 0, data.target);
  ...
  function initGoalInteractions() {
    $(".goal-list").addEventListener("click", (e) => {
      ...
      if (row) openModal(Store.state.goals.find(g => g.id === row.dataset.id));
    });
    $('[data-action="add-goal"]').addEventListener("click", () => openModal(null));
  }
  ...
  function initSettings() {
      ...
      <div class="settings">
        ...
        <section class="card settings__group">
          ...
          <div class="settings__data-btns">
            ...
            <button class="btn btn--danger" type="button" id="set-reset">Reset all data</button>
          </div>
    ...
    /* profile */
    $("#set-name").addEventListener("input", (e) => Store.setUserName(e.target.value));
    /* theme */
    $("#set-theme").addEventListener("click", (e) => {
      const b = e.target.closest("[data-theme-opt]"); if (b) Store.setTheme(b.dataset.themeOpt);
    });
    /* accent */
    $("#set-accent").addEventListener("click", (e) => {
      const b = e.target.closest("[data-accent-opt]"); if (b) { Store.setAccent(b.dataset.accentOpt); toast("Accent updated"); }
    });
    /* animations */
    $("#set-anim").addEventListener("click", () => Store.setAnimations(!Store.state.settings.animations));
    /* data */
    $("#set-export").addEventListener("click", () => {
      downloadFile(`growthos-backup-${dateKey()}.json`, JSON.stringify(Store.state, null, 2));
    ...
    $("#set-reset").addEventListener("click", () => {
      if (confirm("Reset all GrowthOS data? This can't be undone.")) { Store.reset(); toast("Data reset"); }
    });
  ...
  function syncSettingsControls() {
    ...
    $$("#set-theme [data-theme-opt]").forEach(b => b.classList.toggle("is-active", b.dataset.themeOpt === s.settings.theme));
    $$("#set-accent [data-accent-opt]").forEach(b => b.classList.toggle("is-active", b.dataset.accentOpt === s.settings.accent));
    const anim = $("#set-anim"); if (anim) { anim.classList.toggle("is-on", !!s.settings.animations); anim.setAttribute("aria-checked", !!s.settings.animations); }
  }
  ...
  function initConfetti() {
    ...
    const COLORS = ["#4f79ff", "#33d9f2", "#a35bff", "#37d39b", "#ffd15c", "#f2597f"];
    function spawn(n, originX) {
      for (let i = 0; i < n; i++) {
        parts.push({
          x: originX, y: innerHeight * 0.28,
          vx: (Math.random() - 0.5) * 9, vy: Math.random() * -9 - 4,
          g: 0.22 + Math.random() * 0.16, size: 5 + Math.random() * 6,
    ...
    function loop() {
      ...
      parts.forEach(p => { p.vy += p.g; p.x += p.vx; p.y += p.vy; p.vx *= 0.99; p.rot += p.vr; });
